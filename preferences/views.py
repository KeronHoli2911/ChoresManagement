...
def edit_preferences(request):
    ...
    if request.method == 'POST':
        ...
        category_pref_formset = UserCategoryPreferenceFormSet(
            request.POST, instance=user, prefix=prefixes['category_pref']
        )
        experience_formset = UserCategoryExperienceFormSet(
            request.POST, instance=user, prefix=prefixes['experience']
        )
        physical_lim_formset = UserPhysicalLimitationFormSet(
            request.POST, instance=user, prefix=prefixes['limitation']
        )
